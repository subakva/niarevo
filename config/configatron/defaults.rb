configatron.notifier.url_host = 'localhost:3000'
configatron.notifier.from = 'DreamTagger <noreply@niarevo.com>'
configatron.notifier.auto_cc = 'outgoing@niarevo.com'

configatron.asset_hosts.asset_host_url = "niarevo.local:3000"

# from credentials.rb
configatron.newrelic.license_key = ''
configatron.hoptoad.api_key = ''
configatron.recaptcha.public_key = ''
configatron.recaptcha.private_key = ''
configatron.smtp.password = ''
configatron.session.secret = 'dead60b4dab26a286723bb5da73d51bb0073ca5ecf2774f7c94d2fd313034b48171d77d8a0f4487f0773a690e58a3bccefac1d26c079bd567dbf346f9e74984d'

