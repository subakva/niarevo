...
xml.feed 'xmlns' => 'http://www.w3.org/2005/Atom', 'xml:lang' => 'en' do
  ...
  xml.updated Time.zone.now.utc.strftime("%Y-%m-%dT%H:%M:%SZ")
  ...
  @dreams.each do |dream|
    xml.entry do
      ...
      xml.author do
        xml.name dream.user ? dream.user.username : 'Anonymous Dreamer'
        xml.uri dream.user ? user_dreams_url(dream.user.username) : dreams_url
      end
